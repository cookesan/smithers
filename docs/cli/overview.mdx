...
commands[62]:
  ...
  - name: gateway.doctor
    purpose: Validate Gateway readiness before exposing it in production
    flags[3]{name,short,type,default,desc}:
      config,,string,,Path to a JSON GatewayOptions file
      production,,boolean,true,Run production readiness checks
      json,,boolean,false,Print the readiness report as JSON
  - name: token.issue
